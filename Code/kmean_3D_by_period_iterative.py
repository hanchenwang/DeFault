...
draw_catalog=False
draw_pred=True
draw_fault_plane=False
draw_fault_line=True
draw_arrow=False
draw_receiver=True

period_length = 1440  # Change this to your desired period length in days
...
def point_to_plane_distance(plane_coeffs, point):
    # Plane equation coefficients: A, B, C, D in Ax + By + Cz + D = 0
...
# Function to fit a plane using least squares and return its coefficients
def fit_plane_LSE(points):
    G = np.ones((points.shape[0], 3))
    G[:, 0] = points['Longitude']   # X
    G[:, 1] = points['Latitude']   # Y
    Z = points['Depth']         # Z
    (a, b, c), resid, rank, s = np.linalg.lstsq(G, Z, rcond=None)
    plane_coeffs = (a, b, -1, c)
    return plane_coeffs
...
for name, group in grouped:
    ...
    if len(group) >= 5:
        ...
        for cluster, color in zip(range(k), colors):
            ...
            for iteration in range(1):  # Perform refinement for 10 iterations
                # Fit a plane to the current set of cluster points
                plane_coeffs = fit_plane_LSE(cluster_points)

                # Calculate distance of each point from the plane
                # distances = np.array([point_to_plane_distance(plane_coeffs, point) for point in cluster_points])
                distances = np.sqrt(
                ...
                # Keep points that are close to the plane - define your own threshold
                # For example, keep points within the 75th percentile distance
                threshold_distance = np.percentile(distances, 90)
                cluster_points = cluster_points[distances <= threshold_distance]
...
#    fig.savefig(f'Clusters_All_by_{period_str}.png')
    if save_all_periods:
        img_filename = f'Figure_run10_3D_faults_itr/all_cluster_{period_length}_{period_str}.png'
...
if draw_rolling_gif:
    ...
    for azimuth in range(-90,90,1):
        ...
        roll_img_filename = f'Figure_run10_3D_faults_itr/rolling_view/roll_3D_XZ_view_{period_length}_{azimuth}.png'
...
# Create a gif from the png images
    ...
    im = images[0]
    im.save('./Figure_run10_3D_faults_itr/rolling_view/roll_all_clusters_'+str(period_length)+'.gif', save_all=True, append_images=images[1:], optimize=False, duration=100, loop=0)

plt.savefig(f'./Figure_run10_3D_faults_itr/3D_view_{period_length}_orange.png')

# XY
ax.view_init(elev=90, azim=-90)
plt.savefig(f'./Figure_run10_3D_faults_itr/3D_XY_view_{period_length}_orange.png')
# XZ
ax.view_init(elev=0, azim=-90)
plt.savefig(f'./Figure_run10_3D_faults_itr/3D_XZ_view_{period_length}_orange.png')
# YZ
ax.view_init(elev=0, azim=0)
plt.savefig(f'./Figure_run10_3D_faults_itr/3D_YZ_view_{period_length}_orange.png')
#plt.show()
...
# Create a gif from the png images
    ...
    # Append images with the same base
    im = images[0]
    im.save('./Figure_run10_3D_faults_itr/all_clusters_'+str(period_length)+'.gif', save_all=True, append_images=images[1:], optimize=False, duration=100, loop=0)
